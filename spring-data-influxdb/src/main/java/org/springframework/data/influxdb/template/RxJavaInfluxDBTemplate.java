 ...
 */
public class RxJavaInfluxDBTemplate extends AbstractInfluxDBTemplate<RxJavaInfluxDB, Flowable<QueryResult>> {
   ...
   */
  @Override
  public RxJavaInfluxDB newInfluxDB(String url, String username, String password, OkHttpClient.Builder client) {
  ...
  public Flowable<QueryResult> query(Query query) {
    return getInfluxDB().queryRxJava(query, BackpressureStrategy.BUFFER);
  }
  ...
  public Flowable<QueryResult> query(Query query, TimeUnit timeUnit) {
    return getInfluxDB().queryRxJava(query, timeUnit, BackpressureStrategy.BUFFER);
  }
  ...
  public Flowable<QueryResult> query(Query query, int chunkSize) {
    return getInfluxDB().queryRxJava(query, chunkSize, BackpressureStrategy.BUFFER);
  }
