...
public class IniParser extends AbstractParser {
  ...
  public void parse(InputStream input, IniHandler handler) throws IniException {
    parse(newIniSource(input, handler), handler);
  }

  public void parse(Reader input, IniHandler handler) throws IniException {
    parse(newIniSource(input, handler), handler);
  }

  public void parse(URL input, IniHandler handler) throws IniException {
    parse(newIniSource(input, handler), handler);
  }

  private void parse(IniSource source, IniHandler handler) throws IniException {
    handler.startIni();
  ...
  private String parseSectionLine(String line, IniSource source, IniHandler handler) throws IniException {
    String sectionName;
