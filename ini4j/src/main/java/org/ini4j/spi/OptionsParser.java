...
public class OptionsParser extends AbstractParser {
  ...
  public void parse(InputStream input, OptionsHandler handler) throws IniException {
    parse(newIniSource(input, handler), handler);
  }

  public void parse(Reader input, OptionsHandler handler) throws IniException {
    parse(newIniSource(input, handler), handler);
  }

  public void parse(URL input, OptionsHandler handler) throws IniException {
    parse(newIniSource(input, handler), handler);
  }

  private void parse(IniSource source, OptionsHandler handler) throws IniException {
    try {
