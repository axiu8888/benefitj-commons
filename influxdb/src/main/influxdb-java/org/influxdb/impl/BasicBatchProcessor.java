...
public class BasicBatchProcessor {
  ...
  public static final class Builder {
     ...
     * @param jitterInterval the flush jitter interval
     * @param unit           the TimeUnit of the interval
     * @return this Builder to use it fluent
     */
     ...
     */
    public BasicBatchProcessor build() {
      ...
      return new BasicBatchProcessor(this.influxDB, batchWriter, this.threadFactory, this.actions, this.flushIntervalUnit,
  ...
  BasicBatchProcessor(final InfluxDB influxDB,
                      final BatchWriter batchWriter,
                      final ThreadFactory threadFactory,
                      final int actions,
                      final TimeUnit flushIntervalUnit,
                      final int flushInterval,
                      final int jitterInterval,
    ...
    Runnable flushRunnable = new Runnable() {
      ...
      public void run() {
        ...
        write();
        int jitterInterval = (int) (Math.random() * BasicBatchProcessor.this.jitterInterval);
        BasicBatchProcessor.this.scheduler.schedule(this,
                BasicBatchProcessor.this.flushInterval + jitterInterval, BasicBatchProcessor.this.flushIntervalUnit);
      }
    };
    // Flush at specified Rate
    this.scheduler.schedule(flushRunnable,
            this.flushInterval + (int) (Math.random() * BasicBatchProcessor.this.jitterInterval),
  ...
  void write() {
    ...
    try {
      if (this.queue.isEmpty()) {
        BasicBatchProcessor.this.batchWriter.write(Collections.emptyList());
        return;
      }
      //for batch on HTTP.
      ...
      BasicBatchProcessor.this.batchWriter.write(batchKeyToBatchPoints.values());

      for (Entry<Integer, List<String>> entry : udpPortToBatchPoints.entrySet()) {
        for (String lineprotocolStr : entry.getValue()) {
          BasicBatchProcessor.this.influxDB.write(entry.getKey(), lineprotocolStr);
        }
      }
    } catch (Throwable t) {
