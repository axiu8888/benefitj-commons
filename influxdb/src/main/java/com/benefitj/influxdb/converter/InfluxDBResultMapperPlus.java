    ...
    for (Class<?> clazz : classVarAgrs) {
      ...
      fieldMap.forEach((fieldName, field) -> {
        if (field.isAnnotationPresent(Column.class)) {
          influxColumnAndFieldMap.put(field.getAnnotation(Column.class).name(), field);
        } else {
          influxColumnAndFieldMap.put(field.getName(), field);
        }
      });
      getClassFieldCache().put(clazz.getName(), influxColumnAndFieldMap);
    }
  }
