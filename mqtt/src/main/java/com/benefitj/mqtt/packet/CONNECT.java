 ...
 * 客户端连接
 *
 * 客户端连接到服务端后的第一个报文，只能发送一次，服务端返回 {@link MqttMessageType#CONNECT} 报文；
 ...
 *    回一个 0x01 的 {@link MqttMessageType#CONNACK} 响应报文，然后断开客户端连接；
 *
 *
 *  连接标志位:
 *
 ...
 *       网络连接关闭时，服务端发布这个医嘱消息，除非服务端收到 {@link MqttMessageType#DISCONNECT} 报文时删除此遗嘱消息；
 *
 *       遗嘱消息发布的条件，包括但不限于：
 *         (1). 服务端监测到I/O错误，或网络故障；
 *         (2). 客户端在 Keep Alive 的时间内未能通讯；
 *         (3). 客户端未发送 {@link MqttMessageType#DISCONNECT} 报文就关闭了网络连接；
 *         (4). 协议错误，服务端关闭了网络连接；
 *
 *       如果遗嘱标志被设置为1，连接标志中的 Will QoS 和 Will Retain 字段会被服务端用到，
 *       同时，有效载荷中必须包含 Will Topic 和 Will Message 字段；客户端发送了 {@link MqttMessageType#DISCONNECT}
 *       报文，服务端必须将会话中的遗嘱状态移除；
 *
 ...
 *     客户端负责两次报文之间的间隔不超时，客户端可以再任何时候发送 {@link MqttMessageType#PINGREQ} 报文；
 *     如果服务端在1.5倍的保持连接时间内没有收到客户端的报文，服务端必须断开连接；
 *     如果客户端在发送了 {@link MqttMessageType#PINGREQ} 报文之后，在合理的时间内未
 *     收到{@link MqttMessageType#PINGRESP}报文，客户端应该断开连接；
 *
